...
export const category = (app: Application) => {
  ...
  app.service(categoryPath).hooks({
    around: {
      all: [schemaHooks.resolveExternal(categoryExternalResolver), schemaHooks.resolveResult(categoryResolver)]
    },
    before: {
      all: [schemaHooks.validateQuery(categoryQueryValidator), schemaHooks.resolveQuery(categoryQueryResolver)],
      find: [],
      get: [],
      create: [schemaHooks.validateData(categoryDataValidator), schemaHooks.resolveData(categoryDataResolver)],
      patch: [schemaHooks.validateData(categoryPatchValidator), schemaHooks.resolveData(categoryPatchResolver)],
      remove: []
    },
    after: {
      all: []
